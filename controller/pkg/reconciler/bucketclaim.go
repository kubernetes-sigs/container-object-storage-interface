...
func (r *BucketClaimReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		// events to the referencing BucketClaim instead. Reconciles that return nil while waiting
		// on the Bucket rely on these events to eventually fire; there is no backoff-based retry
		// behind them.
		Watches(
			&cosiapi.Bucket{},
