...
func (r *BucketClaimReconciler) reconcile(ctx context.Context, logger logr.Logger, claim *cosiapi.BucketClaim) error {
	...
	if err := r.Get(ctx, bucketNsName, bucket); err != nil {
		...
		if isStaticProvisioning {
			...
			return fmt.Errorf("waiting for Bucket %q to exist", bucketName)
		}
	...
	if !isBound {
		if isStaticProvisioning {
			if err := ensureStaticBucketBound(ctx, logger, r.Client, bucket, claim); err != nil {
				return err
			}
		} else {
...
func ensureStaticBucketBound(
	...
	if ref.UID != claim.UID {
		logger.Info("binding statically-provisioned Bucket to BucketClaim", "claimUID", claim.UID)
		bucket.Spec.BucketClaimRef.UID = claim.UID
